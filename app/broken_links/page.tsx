...
export default function BrokenLinkPage() {
  ...
  if (!analysisData) {
    return <Container>Please run analysis first from the Dashboard.</Container>;
  }
  ...
  return (
    <Container size="md" pt={40}>
      ...
      >
        <span style={{ color: "lightskyblue" }}>Broken Links</span> Report
      </Title>
      {analysisData.map((result) => (
        ...
        >
          <Text weight={500} color="yellow">{result.url}</Text>
          <Text color="red">
            Broken Links: {JSON.stringify(result.error_links)}
          </Text>
        </Paper>
      ))}
    </Container>
